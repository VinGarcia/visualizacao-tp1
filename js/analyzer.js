...
function Analyzer(graphs, side, DEBUG_MODE) {
  ...
  // is 17% of side).
  function isCoordAllowed(coord, coordsVector) {
  ...
  function generateSafeCoordinates(side, coordsVector) {
    ...
    var coord = [
      ...
      self.getRand(side*0.12, side*0.75)
    ];
    var maxIterations = 1000;

    // Compute new random coordinates until we find a valid one.
    while (!self.isCoordAllowed(coord, coordsVector)) {
      coord = [
        self.getRand(side*0.12, side*0.88),
        self.getRand(side*0.12, side*0.75)
      ];
      if(maxIterations-- === 0) {
        throw [
          'Analyzer.generateSafeCoordinates:',
